...
public class WebResultProcessor {
	 ...
	 * @param response
	 */
    public static void render(WebResult result, HttpServletRequest request, HttpServletResponse response) {
		...
		case WebResult.TYPE_VIEW:
/*			ApplicationContextUtils appUtils = ApplicationContextUtils.getInstance();
			ObjectCached<TemplateCachedItem> cache = appUtils.getTemplateCached();
			TemplateCachedItem item = cache.fetch(result.getResult());
			return item.render(request, response);
*/			break;
     ...
     * @param response
     */
	protected static void render(String body, HttpServletResponse response) {
		try {
	 ...
	 * @param response
	 */
	protected static void render(InputStream stream, HttpServletResponse response) {
		try {
